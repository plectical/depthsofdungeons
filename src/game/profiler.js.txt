...
(function () {
  ...
  function rafLoop(now) {
    ...
    if (S.lastRaf > 0) {
      ...
      if (now - S.lastSecTs >= 1000) {
        ...
        S.framesThisSec = 0;
        S.lastSecTs = now;
        // Keep pill aligned with game area
        if (S._repositionWrap) S._repositionWrap();
  ...
  // ─────────────────────────────────────────────
  // POSITION PILL INSIDE GAME AREA
  // ─────────────────────────────────────────────
  let _lastRight = -1;
  function repositionWrap() {
    ...
    // Only reposition if the game area doesn't fill the full viewport width
    const rightOffset = Math.max(0, window.innerWidth - r.right) + 12;
    if (rightOffset !== _lastRight) {
      _lastRight = rightOffset;
      wrap.style.right = rightOffset + 'px';
    }
  }
  // Re-check position periodically until stable (handles Safari late layout)
  repositionWrap();
  setTimeout(repositionWrap, 100);
  setTimeout(repositionWrap, 500);
  setTimeout(repositionWrap, 1500);
  window.addEventListener('resize', repositionWrap);
  // Also reposition on each FPS sample (once per second) via the raf loop
  S._repositionWrap = repositionWrap;
