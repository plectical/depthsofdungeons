...
/**
 * Bloodline tutorial guide — same layout as the Elder dialogue bar
 * (portrait on left, speech on right, tap to continue) but with the
 * bloodline vampire character and purple accent colours.
 */
export function BloodlineGuide({ storageKey, pages, onDone }: Props) {
  const [page, setPage] = useState(0);
  const tip = pages[page]!;
  const isLast = page === pages.length - 1;
  const portraitUrl = useCdnImage('bloodline-guide.jpg');
    ...
    <div style={overlayStyle} onClick={advance}>
      <div style={barStyle}>
        ...
        <div style={portraitContainerStyle}>
          {portraitUrl && <img src={portraitUrl} alt="The Ancestor" style={portraitStyle} />}
        </div>
        {/* Speech bubble */}
        <div style={speechStyle}>
          <div style={nameStyle}>The Ancestor</div>
